...
EchoServer::ProcessPacket()
{
	...
	{
		auto pPacketData = DequePacketData();
		if (pPacketData != nullptr)
		{
			SendMsg(pPacketData->SessionIndex, pPacketData->DataSize, pPacketData->pPacketData);
		}
		else
		{
...
EchoServer::DequePacketData()
{
	...
	PacketDataPtr packetData = mPacketDataQueue.front();
